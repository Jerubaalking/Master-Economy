...
?>
		<td><input name="Tdate[]" size ="12" Type = "date" value=""></td>

		<td><input name="TRECEPT[]"  value="" size ="1"></td>

		<td><input name="Fullname[]"  size ="26" value="<?php echo $Fullname; ?>"></td>
		...
		<td><input name="Branchcode[]" size ="1" value="Loan"></td>
...
<script>
	...
	$('#add_list').click(function(){
		var TRECEPT1 = $('#TRECEPT1').val(),
			Branchcode = $('#Branchcode').val(),
      Tdate1 = $('#Tdate1').val();
			console
		var tr = $('#tr_clone tr').clone()
		tr.find('[name="TRECEPT1[]"]').val(TRECEPT1)
		tr.find('[name="Branchcode[]"]').val(Branchcode)
		tr.find('[name="Tdate1[]"]').val(Tdate1)
		tr.find('.TRECEPT1').html($('#TRECEPT1 option[value="'+TRECEPT1+'"]').html())
		tr.find('.Branchcode').html($('#Branchcode option[value="'+Branchcode+'"]').html())
		tr.find('.Tdate1').html(Tdate1)
		$('#attendance-list tbody').append(tr)
		$('#TRECEPT1').val('').select2({
			placeholder:"Select here",
			width:"100%"
		})
		$('#Branchcode').val('')
	...
	$(document).ready(function(){
		$('#employee-attendance').submit(function(e){
				...
				start_load();
			$.ajax({
				url:'ajax.php?action=save_employee_attendance1',
				method:"POST",
				data:$(this).serialize(),
