...
if(isset($_POST['save']))
{
	try{
		...
		{
		$insert1 = mysqli_query($conn, "INSERT INTO twallet VALUES('','$tid','$amt')") or die (mysqli_error($conn));
		$insert2 = mysqli_query($conn, "INSERT INTO mywallet VALUES('','$tid','NIL','$amt','$desc','$wtype',NOW())") or die (mysqli_error($conn));
		...
		$Get_amt3 = $row3['Total'];
		$Total =  $Get_amt3 + $amt;
		$update3 = mysqli_query($conn, "UPDATE twallet SET Total='$Total' WHERE tid = '$tid'") or die (mysqli_error($conn));
		$insert1 = mysqli_query($conn, "INSERT INTO mywallet VALUES('','$tid','NIL','$amt','$desc','$wtype',NOW())") or die (mysqli_error($conn));
		if(!($update3 && $insert1))
		{
		...
		$update = mysqli_query($conn, "UPDATE twallet SET Total = '$Get_amt' WHERE tid = '$tid'") or die (mysqli_error($conn));
		$insert = mysqli_query($conn, "INSERT INTO mywallet VALUES('','$tid','NIL','$amt','$desc','$wtype',NOW())") or die (mysqli_error($conn));
		if(!($update && $insert))
		{
	  ...
	  <div class="modal-content">
        ...
        <div class="modal-body">
			...
			<div class="form-group">
				...
				<?php
				$get = mysqli_query($conn, "SELECT * FROM user WHERE id != '$tid'") or die (mysqli_error($conn));
...
if(isset($_POST['transfer']))
{
	try{
		...
		$slec = mysqli_query($conn, "SELECT * FROM twallet WHERE tid = '$transfer_to'") or die (mysqli_error($conn));
		$have = mysqli_fetch_array($slec);

		$slecin = mysqli_query($conn, "SELECT * FROM twallet WHERE tid = '$tid'") or die (mysqli_error($conn));
		$havein = mysqli_fetch_array($slecin);
		$Total = $havein['Total'];
		$Balance = $Total - $amount;
		if(mysqli_num_rows($slec)==0)
		{
		$update2 = mysqli_query($conn, "UPDATE twallet SET Total = '$Balance' WHERE tid = '$tid'") or die (mysqli_error($conn));
		$insert2 = mysqli_query($conn, "INSERT INTO twallet VALUES('','$transfer_to','$amount')") or die (mysqli_error($conn));
		$insert2 = mysqli_query($conn, "INSERT INTO mywallet VALUES('','$tid','$transfer_to','$amount','$desc','$wtype',NOW())") or die (mysqli_error($conn));
		...
		$insert = mysqli_query($conn, "INSERT INTO mywallet VALUES('','$tid','$transfer_to','$amount','$desc','$wtype',NOW())") or die (mysqli_error($conn));
		$slect = mysqli_query($conn, "SELECT * FROM twallet WHERE tid = '$transfer_to'") or die (mysqli_error($conn));
...
?>
 ...
 <?php
$tid = $_SESSION['tid'];
$select = mysqli_query($conn, "SELECT * FROM mywallet") or die (mysqli_error($conn));
while($row = mysqli_fetch_array($select))
{
$id = $row['id'];
$amt = $row['Amount'];
