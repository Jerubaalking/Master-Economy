...
Class Action {
	...
	function save_employee_attendance1(){
		extract($_POST);
		foreach($TRECEPT as $k =>$v){
			$Tdate[$k] =date("Y-m-d",strtotime($Tdate[$k]));
			$data =" TRECEPT='$TRECEPT[$k]' ";
			$data .=", Branchcode = '$Branchcode[$k]' ";
			$data .=", Tdate = '$Tdate[$k]' ";
			$data .=", Cramount = '$ScheduleAmount[$k]' ";
			$data .=", Fullname = '$Fullname[$k]' ";
			$data .=", GroupNo = '$GroupNo[$k]' ";
			$data .=", ACCNO = '$ACCNO[$k]' ";
			$data .=", LoanID = '$LoanID[$k]' ";
			$save[] = $this->db->query("INSERT INTO trans set ".$data);
		}
		...
		function save_loan(){
			...
			foreach($TRECEPT as $k =>$v){
				...
				$data .=", Branchcode = '$Branchcode[$k]' ";
				$data .=", Tdate = '$Tdate[$k]' ";
				$data .=", Dramount = '$ScheduleAmount[$k]' ";
				$data .=", Fullname = '$Fullname[$k]' ";
				$data .=", GroupNo = '$GroupNo[$k]' ";
				$data .=", ACCNO = '$ACCNO[$k]' ";
				$data .=", LoanID = '$LoanID[$k]' ";
				$data .=", ScheduleAmount = '$Orgin[$k]' ";
				$data .=", Special = '$Special[$k]' ";
				$data .=", Scheduleday = '$Scheduleday[$k]' ";
				$data .=", Iamount = '$Iamount[$k]' ";
				$save[] = $this->db->query("INSERT INTO trans set ".$data);
			}
			if(isset($save))
				return 1;
		...
		function save_expense(){
			...
			foreach($TRECEPT as $k =>$v){
				$Tdate[$k] =date("Y-m-d",strtotime($Tdate[$k]));
				$data =" TRECEPT='$TRECEPT[$k]' ";
				$data .=", Typecode  = 'EXPENSE' ";
				$data .=", Branchcode = '$Branchcode[$k]' ";
				$data .=", Tdate = '$Tdate[$k]' ";
				$data .=", Dramount = '$ScheduleAmount[$k]' ";
				$data .=", Description = '$descript[$k]' ";
				$save[] = $this->db->query("INSERT INTO trans set ".$data);
			}
