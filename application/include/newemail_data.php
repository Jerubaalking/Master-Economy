...
$query = mysqli_query($conn, "SELECT email FROM systemset") or die (mysqli_error($conn));
...
{
	$insert = mysqli_query($conn, "INSERT INTO etemplates VALUES('','$tid','$email','$subj','$msg',NOW())") or die (mysqli_error($conn));
	echo "<hr>";
...
{
$tid = $_SESSION['tid'];
$select = mysqli_query($conn, "SELECT * FROM user WHERE id = '$tid'") or die (mysqli_error($conn));
...
$insert = mysqli_query($conn, "INSERT INTO etemplates VALUES('','$tname','$template','$desc','$created_by')") or die (mysqli_error($conn));
if(!$insert)
{
