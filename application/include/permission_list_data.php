...
<form method="post">
	...
	<hr>
			 ...
			 <table id="example1" class="table table-bordered table-striped">
                ...
                <tbody>
<?php
$select = mysqli_query($conn, "SELECT DISTINCT(tid) FROM emp_permission") or die (mysqli_error($conn));
...
//$id = $row['id'];
$tid = $row['tid'];
$getin = mysqli_query($conn, "SELECT name FROM user WHERE id = '$tid'") or die (mysqli_error($conn));
...
<?php
						...
						echo "<script>window.location='permission_list.php?id=".$_SESSION['tid']."&&mid=".base64_encode("413")."'; </script>";
							...
							for($i=0; $i < $N; $i++)
							{
								$result = mysqli_query($conn,"DELETE FROM emp_permission WHERE tid ='$id[$i]'");
