...
<?php
$id = $_GET['id'];
$select = mysqli_query($conn, "SELECT * FROM borrowers WHERE id = '$id'") or die (mysqli_error($conn));
...
?>
		...
		<div class="form-group">
		 ...
		 <div class="form-group">
                  	...
                  	<div class="col-sm-10"><textarea name="addrs1"  class="form-control" rows="4" cols="80"><?php echo $row['addrs1']; ?></textarea></div>
          </div>

			<div class="form-group">
                  	<label for="" class="col-sm-2 control-label" style="color:#009900">Address 2</label>
                  	<div class="col-sm-10"><textarea name="addrs2"  class="form-control" rows="4" cols="80"><?php echo $row['addrs2']; ?></textarea></div>
          	</div>
		...
		<div class="form-group">
                  ...
                  <div class="col-sm-10">
                  <input name="zip" type="text" class="form-control" value="<?php echo $row['zip']; ?>" placeholder="Zip Code" >
                  </div>
                  </div>
...
$addrs2 = mysqli_real_escape_string($conn, $_POST['addrs2']);
...
if($image == "")
{
	$insert = mysqli_query($conn, "UPDATE borrowers SET fname='$fname', lname='$lname', email='$email', phone='$phone', addrs1='$addrs1', addrs2='$addrs2', city='$city', state='$state', zip='$zip', country='$country', comment='$comment', account='$account' WHERE id = '$id'") or die (mysqli_error($conn));
	if(!$insert)
	{
	...
	elseif($imageFileType != "jpg" && $imageFileType != "png" && $imageFileType != "jpeg" && $imageFileType != "gif")
	{
		...
		$insert = mysqli_query($conn, "UPDATE borrowers SET fname='$fname', lname='$lname', email='$email', phone='$phone', addrs1='$addrs1', addrs2='$addrs2', city='$city', state='$state', zip='$zip', country='$country', comment='$comment', account='$account', image='$location' WHERE id = '$id'") or die (mysqli_error($conn));
		if(!$insert)
		{
			echo "<div class='alert alert-info'>Update Error.....Please try again later</div>";
		}
