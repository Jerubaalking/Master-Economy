...
<?php
$tid = $_SESSION['tid'];
$select = mysqli_query($conn, "SELECT * FROM message WHERE sender_id = '$tid'") or die (mysqli_error($conn));
...
$date = $row['date_time'];
$select1 = mysqli_query($conn, "SELECT name FROM user WHERE id = '$recipient_id'") or die (mysqli_error($conn));
...
<?php
						...
						echo "<script>window.location='outboxmessage.php?id=".$_SESSION['tid']."'; </script>";
							...
							for($i=0; $i < $N; $i++)
							{
								$result = mysqli_query($conn,"DELETE FROM message WHERE id ='$id[$i]'");
