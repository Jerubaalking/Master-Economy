...
<?php
$id = $_GET['id'];
$select = mysqli_query($conn, "SELECT * FROM loan_info WHERE id = '$id'") or die (mysqli_error($conn));
...
?>
			 ...
			 <div class="form-group">
                <label for="" class="col-sm-2 control-label" style="color:#009900">Borrower</label>
				 <div class="col-sm-10">
				<?php
				$get = mysqli_query($conn, "SELECT * FROM borrowers WHERE id = '$borrower'") or die (mysqli_error($conn));
...
<?php
$id = $_GET['id'];
$search = mysqli_query($conn, "SELECT * FROM additional_fees WHERE get_id = '$id'") or die (mysqli_error($conn));
...
<div align="left">
   ...
   <?php
						...
						echo "<script>window.location='updateloans.php?id=".$idm."&&mid=".base64_encode("405")."'; </script>";
							...
							for($i=0; $i < $N; $i++)
							{
								$result = mysqli_query($conn,"DELETE FROM additional_fees WHERE id ='$id[$i]'");
								echo "<script>window.location='updateloans.php?id=".$idm."&&mid=".base64_encode("405")."'; </script>";
							}
							}
							}
...
$id = $_GET['id'];
$tid = $_SESSION['tid'];
$insert = mysqli_query($conn, "INSERT INTO additional_fees(id,get_id,tid,fee,Amount) VALUES('','$id','$tid','','')") or die (mysqli_error($conn));
if(!$insert)
{
...
$update = mysqli_query($conn, "UPDATE additional_fees SET fee = '$fee', Amount = '$amount' WHERE id = '$s'") or die (mysqli_error($conn));
$i++;
if(!$update)
{
...
$insert = mysqli_query($conn, "INSERT INTO attachment(id,get_id,tid,attached_file,date_time) VALUES('','$id','$tid','$newname',NOW())") or die (mysqli_error($conn));
if(!$insert)
{
echo "<script>alert('Record not inserted.....Please try again later!'); </script>";
}
...
<?php
$id = $_GET['id'];
$search = mysqli_query($conn, "SELECT * FROM collateral WHERE idm = '$id'") or die (mysqli_error($conn));
...
$type_of_collateral = mysqli_real_escape_string($conn, $_POST['type_of_collateral']);
...
$proof_of_ownership = mysqli_real_escape_string($conn, $_POST['proof_of_ownership']);
...
$insert = mysqli_query($conn, "INSERT INTO collateral VALUES('','$id','$tid','$name','$type_of_collateral','$model','$make','$serial_number','$estimated_price','$proof_of_ownership','$cimage','$observation')") or die (mysqli_error($conn));
if(!$insert)
{
echo "<script>alert('Record not inserted.....Please try again later!'); </script>";
}
...
<?php
$id = $_GET['id'];
$searchin = mysqli_query($conn, "SELECT * FROM pay_schedule WHERE get_id = '$id'") or die (mysqli_error($conn));
...
<div align="left">
   ...
   <?php
						...
						echo "<script>window.location='updateloans.php?id=".$idm."&&mid=".base64_encode("405")."'; </script>";
							...
							for($i=0; $i < $N; $i++)
							{
								$result = mysqli_query($conn,"DELETE FROM pay_schedule WHERE id ='$id[$i]'");
								echo "<script>window.location='updateloans.php?id=".$idm."&&mid=".base64_encode("405")."'; </script>";
							}
							}
							}
...
$id = $_GET['id'];
$tid = $_SESSION['tid'];
$insert = mysqli_query($conn, "INSERT INTO pay_schedule(id,get_id,tid,schedule,balance,interest,payment) VALUES('','$id','$tid','','','','')") or die (mysqli_error($conn));
if(!$insert)
{
...
{
$tid = $_SESSION['tid'];
$term = mysqli_real_escape_string($conn, $_POST['term'][$i]);
$day = mysqli_real_escape_string($conn, $_POST['d1'][$i]);
$schedule_of_paymt = mysqli_real_escape_string($conn, $_POST['schedule'][$i]);
$interest = mysqli_real_escape_string($conn, $_POST['interest'][$i]);
$penalty = mysqli_real_escape_string($conn, $_POST['penalty'][$i]);
$schedule = mysqli_real_escape_string($conn, $_POST['schedulek'][$i]);
$balance = mysqli_real_escape_string($conn, $_POST['balance'][$i]);
$interest = mysqli_real_escape_string($conn, $_POST['interest'][$i]);
$payment = mysqli_real_escape_string($conn, $_POST['payment'][$i]);

$update = mysqli_query($conn, "UPDATE pay_schedule SET schedule = '$schedule', balance = '$balance', interest = '$interest', payment = '$payment' WHERE id = '$s'") or die (mysqli_error($conn));
$insert = mysqli_query($conn, "INSERT INTO payment_schedule VALUES('','$s','$tid','$term','$day','$schedule_of_paymt','$interest','$penalty')") or die (mysqli_error($conn));
$insert = mysqli_query($conn, "UPDATE loan_info SET upstatus = 'Completed' WHERE id = '$idm'") or die (mysqli_error($conn));
if(!($update && $insert))
{
echo "<script>alert('Record not inserted.....Please try again later!'); </script>";
}
