...
<?php
$id = $_GET['id'];
$search = mysqli_query($conn, "SELECT * FROM fin_info WHERE get_id = '$id'") or die (mysqli_error($conn));
...
<div align="left">
   ...
   <?php
						...
						echo "<script>window.location='updateborrowers.php?id=".$idm."&&mid=".base64_encode("403")."'; </script>";
							...
							for($i=0; $i < $N; $i++)
							{
								$result = mysqli_query($conn,"DELETE FROM fin_info WHERE id ='$id[$i]'");
								echo "<script>window.location='updateborrowers.php?id=".$idm."&&mid=".base64_encode("403")."'; </script>";
							}
							}
							}
...
$id = $_GET['id'];
$tid = $_SESSION['tid'];
$insert = mysqli_query($conn, "INSERT INTO fin_info(id,get_id,tid,occupation,mincome) VALUES('','$id','$tid','','')") or die (mysqli_error($conn));
if(!$insert)
{
...
$update = mysqli_query($conn, "UPDATE fin_info SET occupation = '$fee', mincome = '$amount' WHERE id = '$s'") or die (mysqli_error($conn));
$i++;
if(!$update)
{
...
$insert = mysqli_query($conn, "INSERT INTO battachment(id,get_id,tid,attached_file,date_time) VALUES('','$id','$tid','$newname',NOW())") or die (mysqli_error($conn));
$insert = mysqli_query($conn, "UPDATE borrowers SET status = 'Completed' WHERE id = '$id'") or die (mysqli_error($conn));
if(!$insert)
{
echo "<script>alert('Record not inserted.....Please try again later!'); </script>";
}
