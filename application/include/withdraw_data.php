...
<?php
$search = mysqli_query($conn, "SELECT * FROM borrowers");
...
if(isset($_POST['save']))
{
	try{
		...
		$google_details = mysqli_query($conn, "SELECT * FROM borrowers WHERE account = '$account'");
		while($get_details = mysqli_fetch_array($google_details))
		{
			...
			if($bal < $amount)
			{
				...
				$update = mysqli_query($conn, "UPDATE borrowers SET balance = '$total' WHERE account = '$account'") or die (mysqli_error($conn));
				$insert = mysqli_query($conn, "INSERT INTO transaction VALUES('','$txid','Withdraw','$account','$fn','$ln','$em','$ph','$amount',NOW())") or die (mysqli_error($conn));
				if(!($update && $insert))
				{
				echo "<div class='alert alert-info'>Unable to Process Transaction.....Please try again later</div>";
				}
				else{
					$sql_alert = mysqli_query($conn, "SELECT * FROM sms") or die (mysqli_error($conn));
